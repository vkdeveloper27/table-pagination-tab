...
export class PieChartComponent implements OnInit {
  ...
  createPieChart() {
    ...
    //   })
    //   .text(function (d) {
    //     return d.data.name; // display the actual data point label
    //   })
    //   .style('text-anchor', function (d) {
    //     return midAngle(d) < Math.PI ? 'start' : 'end';
    //   })
    ...
    // // Connect the lines to the labels
    // polyline.each(function(d) {
    //   var polyline = d3.select(this);
    //   var text = g.select('.name-text[data-id="'+d.data.name+'"]');
    //   polyline.attr('data-id', d.data.name)
    //           .attr('id', 'line-'+d.data.name);
    //   text.attr('data-id', d.data.name)
    ...
    var text = g
      ...
      .text(function (d) {
        return d.data.value; // display the actual data point label
      })
      .style('text-anchor', function (d) {
        return midAngle(d) < Math.PI ? 'start' : 'end';
      })
    ...
    var polyline = g
      ...
      .attr('stroke-dasharray', '5,5'); // add dashed lines for increased visibility
    ...
    var box = g
      ...
      .append('rect')
      .attr('class', 'box')
      .attr('width', 10) // set the width of the box
      .attr('height', 10) // set the height of the box
      ...
      .attr('x', function (d) {
        var pos = labelArc.centroid(d);
        pos[0] = radius * 0.95 * (midAngle(d) < Math.PI ? 1 : -1);
        return pos[0] - 5; // position the box at the end of the line
      })
      .attr('y', function (d) {
        var pos = labelArc.centroid(d);
        pos[0] = radius * 0.95 * (midAngle(d) < Math.PI ? 1 : -1);
        return pos[1] - 5; // position the box at the end of the line
      });

    // increase the height of the chart to make the data more visible
